			...
			<div class="col-md-12 col-sm-12">
				...
				<div class="col-md-9 col-sm-9">
					<div class="section-showproduct-right">
						<%
							CustomerServiceImp cusdservice = new CustomerServiceImp();
							int page_one = 1;
							int recordsPerPage = 10;
							if (request.getParameter("page") != null)
								page_one = Integer.parseInt(request.getParameter("page"));
							List<Post> list = cusdservice.getpostList((page_one - 1) * recordsPerPage, recordsPerPage);
							int numberof_records = cusdservice.get_totalrecords();
							int numberof_page = (int) Math.ceil(numberof_records * 1.0 / recordsPerPage);
							for (Post p : list) {
						%>
						<div class="section-element">
							<div class="col-md-3 col-sm-3">
								<img style="height: 110px;" src="uploads/<%=p.getImage()%>"
									alt="ICON-PAY" />
							</div>
							<div class="col-md-4 col-sm-4">
								<input type="hidden" id="prname" value="<%=p.getProductname()%>">
								<h4 style="font-weight: bold; color: black;font-size:15px;">
									Product Name:
									<%=p.getProductname()%></h4>
								<h6 style="font-weight: bold; color: black; font-size: 12px;">
									Type of Shop:
									<%=p.getTypeofshop()%></h6>
								<h6
									style="font-weight: bold; color: black; font-size: 12px; margin-bottom: 11px;">
									Location:
									<%=p.getLocation() %></h6>
								<h5>
									Post Date:
							...
							<div class="col-md-2 col-sm-2">
								<h5 style="margin-top: 10px; font-weight: bold;">Quantity</h5>
								<h4 style="width: 50px; margin-bottom: 20px;"><%=p.getQuantity()%></h4>
								<%
									if (p.getQuantity() <= 0) {
								%>
								<h6 style="color: red; font-weight: bold; font-size: 15px;">Not
									Available</h6>
								<%
									} else {
								%>
								<h6 style="color: green; font-weight: bold; font-size: 15px;">Available</h6>
								<%
									}
								%>
							</div>
							<div class="col-md-3 col-sm-3">
								<h4>
									<a style="margin-left: 40px;"
										href="shopkeeperDetails.jsp?ownerid=<%=p.getEmail()%>">Contact
										shopkeeper</a>
								</h4>
								<h5
									style="margin-left: 45px; margin-bottom: 6px; font-weight: bold;"><%=p.getPrice()%>.Tk
								</h5>
								<br /> <a
									style="padding: 5px; background: #74B54D; border-radius: 3px; color: #D0DDC2; margin-left: 45px;"
									href="addcart.jsp?postId=<%=p.getId()%>">Order Now</a>
							</div>
						</div>
						<%
							}
					...
					<ul class="pagination">
						...
						%>
						<a
							style="text-decoration: none; padding: 3px; background: white; color: black; border-radius: 5px; border: 1px solid #ddd;"
							href="product.jsp?page=<%=page_one - 1%>">previous</a>
						<%
							}
						%>
						<%
							for (int j = 1; j <= numberof_page; j++) {
						%>
						<a href="product.jsp?page=<%=j%>"><%=j%></a>
						<%
							}
						%>
						<a
							style="text-decoration: none; padding: 3px; background: white; color: black; border-radius: 5px; border: 1px solid #ddd;"
							href="product.jsp?page=<%=numberof_page%>">Last Page</a>
					</ul>
				</div>
			</div>
		</div>
